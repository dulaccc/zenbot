...
module.exports = function container (get, set, clear) {
  return function notifier (bot_token, user_ids, strategy_options) {

    // NB: do not store the `strategy_options` instance, to avoid retain cycles and memory leaks
    var mode = strategy_options.mode
    ...
    var onUp = function() {
      var text = ""
      text += "I'm up on *" + strategy.toUpperCase() + "* , period " + period_size + ", mode _" + mode + "_"
      user_ids.forEach(function(uid) {
        bot.sendMessage(uid, text, {parse: 'Markdown'})
      })
    }
