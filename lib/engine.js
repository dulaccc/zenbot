...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      if (so.notify === true) {
        // Publish signal on Telegram
        var notifier = get('lib.notifier')(c.telegram.bot.token, c.telegram.user.id)
        notifier.notify_signal(s, so.strategy, so.period)
      }
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          if (so.notify === true) {
            // Publish trade on Telegram
            var notifier = get('lib.notifier')(c.telegram.bot.token, c.telegram.user.id)
            notifier.notify_trade('buy_order', my_trade, s)
          }
        ...
        if (so.mode === 'live' || trade.price >= s.sell_order.price) {
          ...
          if (so.notify === true) {
            // Publish trade on Telegram
            var notifier = get('lib.notifier')(c.telegram.bot.token, c.telegram.user.id)
            notifier.notify_trade('sell_order', my_trade, s)
          }
